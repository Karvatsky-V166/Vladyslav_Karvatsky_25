...
- vstup: N (počet čísel)
- checkbox: "Nahradiť párne" -> ak je zapnutý, párne čísla sa nahradia 0
- tlačidlo: Generovať
- zobrazí zoznam čísel a graf (matplotlib)
"""

import random
import PySimpleGUI as sg
import matplotlib
matplotlib.use('TkAgg')   # zabezpečuje funkčnosť s PySimpleGUI/Tk
...
layout = [
    ...
    [sg.Checkbox('Nahradiť párne čísla (0)', key='-REPLACE-')],
...
while True:
    ...
    if event == 'Generovať':
        ...
        try:
            n = int(n_str)
            if n <= 0:
                sg.popup_error('Zadajte kladné celé číslo!')
                continue
            if n > 100000:
                if not sg.popup_ok_cancel('N je veľmi veľké (>' + str(100000) + '). Pokračovať?'):
                    continue
        except:
    ...
    if event == '-SAVE-':
        if not current_nums:
            ...
            continue
        save_path = sg.popup_get_file('Uložiť súbor', save_as=True, no_window=True,
        ...
        if save_path:
            try:
                ...
                sg.popup('Súbor uložený:', save_path)
